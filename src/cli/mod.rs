...
struct JsonOutput {
    ...
    modifications: Option<Vec<crate::graft::Modification>>,
    #[serde(skip_serializing_if = "Option::is_none")]
    error: Option<String>,
}
...
mod tests {
    ...
    #[test]
    fn test_cli_parse() {
        let args = vec!["graft", "file.rs", "-q", "query", "-t", "template"];
        let cli = Cli::try_parse_from(args).unwrap();
        assert_eq!(cli.files, vec!["file.rs".to_string()]);
        assert_eq!(cli.query, vec!["query".to_string()]);
        assert_eq!(cli.template, vec!["template".to_string()]);
    }
}
